...
// ✅ Ensure only NGO can access
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'NGO') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'verify') {
        ...
        ");
        $stmt->bind_param("ii", $user_id, $totalCredits);
        $stmt->execute();
        $stmt->close();

        $message = "✅ Proof ID $proof_id verified successfully!";
        ...
        $message = "❌ Proof ID $proof_id rejected!";
    }
}

// ✅ Fetch all pending proofs (not yet verified/rejected)
$sql = "(
    SELECT p.id, p.tree_species, p.geo_location, p.tree_count, p.land_type, 
            p.photo_url, p.qrcode_file, p.uploaded_at, f.name AS owner_name
        FROM planting_proofs p
        JOIN factories f ON p.user_id = f.id
        WHERE p.id NOT IN (SELECT proof_id FROM verifications)
    )
    UNION
    (
        SELECT p.id, p.tree_species, p.geo_location, p.tree_count, p.land_type, 
            p.photo_url, p.qrcode_file, p.uploaded_at, u.name AS owner_name
        FROM planting_proofs p
            ...
            <div class="max-w-7xl mx-auto px-6 py-4">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-4">
                        <span class="text-gray-600">Welcome, NGO</span>
                        <!-- <a href="proof_history.php" class="flex items-center px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm text-black"><?= $row['id'] ?></td>
                                    <td class="px-4 py-3 text-sm text-black"><?= htmlspecialchars($row['owner_name']) ?></td>
                                    <td class="px-4 py-3 text-sm text-black"><?= htmlspecialchars($row['tree_species']) ?></td>
                                    <td class="px-4 py-3 text-sm text-black">
                                        <?= htmlspecialchars($row['geo_location']) ?>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-black"><?= $row['tree_count'] ?></td>
                                    <td class="px-4 py-3 text-sm text-black"><?= $row['land_type'] ?></td>
                                    <td class="px-4 py-3 text-sm text-black"><?= $row['uploaded_at'] ?></td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex gap-2">
                                            <form method="post">
                                                ...
                                                <button onclick="showVerifyModal()" type="submit" name="action" value="verify"
                                            ...
                                            <form method="post">
                                                <input type="hidden" name="proof_id" value="<?= $row['id'] ?>">
                                                <button onclick="showRejectModal()" type="submit" name="action" value="reject"
